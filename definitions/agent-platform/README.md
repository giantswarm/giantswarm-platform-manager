...
| `migrations.yaml` | Keys the definition renders that an installation enabled before a migration lacks (a referenced client Secret, the portal's Component, the edge's JWT provider, …), each with the migration that adds it: a leaf the record lacks under one of them is a planned change, not drift. |
