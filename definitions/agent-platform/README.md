...
| `removals.yaml` | Keys an enabled installation carries today that the definition does not render, each with the reason it is dropped: the template renders it, it becomes a referenced Secret, the platform does not read it, the customer-portal definition owns it, or it is a named migration (M12…) of a former input. A key of kind `kept` is not dropped: its replacement is not on the fleet yet (muster's trusted issuers the shared defaults do not derive, a connector pin, the kagent UI's ingress peer, inlined ModelConfigs, tuned resources), so the plan carries it from the record into the render, comments included, and the comparison finds it as defined; the reason says what has to land before the key goes. |
| `migrations.yaml` | Keys the definition renders that an installation enabled before a migration lacks (a referenced client Secret, the hub's token-exchange client in a spoke's Dex, the portal's Component, the edge's JWT provider, …), each with the migration that adds it: a leaf the record lacks under one of them is a planned change, not drift; so is a comma-separated scalar (kagent's `oidc-extra-audience`) whose only change is entries named as `<path>[<entry>]`. |
| `features.yaml`, `probes.yaml` | The consistency features and their dimensions — a file dimension's key names the files and YAML paths it observes in the grammar the file's header documents, `catchAll: true` marks the kind's dimension for every other leaf, and what no dimension names the verify reports under the feature `other`; the live probes the verify runs. |
...
| Which lists the installation's encrypted Dex values still carry, the other prerequisite of the referenced Dex client secrets (below) | `installation.dexSecretLists`: the lists of `installations/<name>/apps/dex-app/secret-values.yaml.patch` the values merge takes whole over the plaintext patch — `oidc.extraStaticClients` (the hand-registered clients with their inline secrets) and `oidc.staticClients.dexK8SAuthenticator.trustedPeers` — each with its number of entries, read from the keys and the list shape of the SOPS-encrypted file; nothing is decrypted. None where the patch carries none or does not exist; an unreadable patch puts the error on the report. Renders nothing; where the plan renders one of those lists and the encrypted values carry it, the comparison runs and the commit is held (`commitRefused`) |
...
- for a component whose list the record does not set, the App's chart enables them by default: the cluster chart
  ...
  the record sets the gates). The chart's version is the App's `spec.version` where it pins one; a release-based
  App — every CAPA and CAPZ management cluster — pins none and names its release in the values
  (`global.release.version`), and the version is the one that release lists for the chart in
  [giantswarm/releases](https://github.com/giantswarm/releases) (`<provider>/v<release>/release.yaml`, read as the
  person). A release that cannot be read or lists no such chart leaves the fact `false` with the reason on the
  report; the installation stays readable and the comparison runs.
...
path and no Kubernetes object is rendered by two definitions. That is a prerequisite of the plan: with `installation.dexAppVersion` on record older than 3.2.2, the comparison still shows every difference and the commit is held — `commitRefused` names the version, the file it was read from and the collections kustomization to pin `dex-app` in first; an empty fact holds nothing. The other prerequisite is the layout of the installation's Dex values: app-operator merges the plaintext configmap patch with the encrypted secret patch (helm-controller its `valuesFrom`) by replacing a list, never by appending, so an `oidc.extraStaticClients` or `dexK8SAuthenticator.trustedPeers` list the encrypted values still carry stands whole over the one this definition renders and the platform's clients never reach Dex. With `installation.dexSecretLists` naming such a list, the comparison still shows every difference and the commit is held — `commitRefused` names the encrypted file, the lists with their entries and the rendered clients and peers they would shadow; the engine never edits the encrypted file, so a person carries every entry over first — each hand-registered client to its own Secret and a plaintext entry the plan keeps, each peer to the plaintext list — and drops the lists from the encrypted values (`removals.yaml`, M1). What a person supplies at commit is
