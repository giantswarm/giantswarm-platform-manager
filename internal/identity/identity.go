...
// token muster obtained for them through the App giantswarm-platform-manager
// and puts on every call — and that token, which every GitHub call of the
// request runs with. The manager does nothing on GitHub as itself for a
// person — every write lands as the caller, bounded to the person's rights ∩
// the App's — so a context without an identity only exists in tests and in a
// server that runs without OAuth.
package identity
...
// runs for the App giantswarm-platform-manager, once per person.
