...
func TestTestInstallationIsTheWavesFirstStage(t *testing.T) {
	...
	order := waveOrder([]installations.Report{{Installation: installations.Installation{Name: rowan, Customer: "acme"}}, {Installation: installations.Installation{Name: rtHub, Customer: rtOwnCustomer}}, {Installation: installations.Installation{Name: rtGarm, Customer: rtOwnCustomer}}}, hub)
...
func TestNoReviewDecidesAndTellsOneSentence(t *testing.T) {
	...
	if got, want := standupText(a, stageMerge{by: "alice", stage: rtGarm}), "*alice* reconciled *agent-platform* on *garm*."; got != want {
		t.Fatalf("standup text %q, want %q", got, want)
	}
	a.Spec.Kind = actions.KindEnable
	if got, want := standupText(a, stageMerge{by: "alice", stage: rtGarm}), "*alice* enabled *agent-platform* on *garm*."; got != want {
		t.Fatalf("standup text %q, want %q", got, want)
	}
