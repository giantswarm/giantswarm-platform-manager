...
const (
	...
	// fieldChatKey is the AI chat's Anthropic API key and
	// fieldChatGoogleCredentials a Vertex chat's service-account JSON, named
	// as the agent-platform definition names them: one value under one field,
	// supplied to whichever definition's file carries it.
	fieldChatKey               = "aiChat.anthropic.apiKey"       // #nosec G101 -- a field name, not a value
	fieldChatGoogleCredentials = "aiChat.google.credentialsJson" // #nosec G101 -- a field name, not a value
)

// chatProviderVertex is platformSection.chatProvider of a chat that runs
// Claude on Vertex AI.
const chatProviderVertex = "vertex"
...
// carries app.extensions), whether the portal runs the AI chat and on which
// provider, whether the Component's credentials Secret for the chat is on
// record, and the keys of the section the portal's own app-config carries, at
// their paths with their values; see the schema.
type PlatformSection struct {
	...
	ChatCredentials bool           `json:"chatCredentials"`
...
// at commit, by field name: the secrets — the AI chat's credential among
// them while it is the portal's (chatCredentialField) — and the GitHub App's
// id, which lives only in the encrypted file. Everything else the portal
// needs is generated by the commit step from the placeholders in the fileset.
func (in *Input) suppliedSecretFields() []string {
	...
	if field := in.chatCredentialField(); field != "" {
...
// chatCredentialField is the supplied field of the AI chat's credential the
// portal's own environment carries, or "". Where the platform runs and the
// record carries the chat, the credential is the portal's until the
// agent-platform Component's credentials Secret is on record: a hand-kept
// portal keeps it in its user secrets, and each step that moves the chat onto
// the Component — this definition's commit, the agent-platform reconcile,
// this definition's hand-over of the section — writes user-secrets with it,
// or keeps the file on record that holds it, until the Component's Secret
// carries it (the agent-platform definition's M18, once the portal's
// app-config no longer carries the chat). Its Anthropic API key, or on
// Vertex AI the service account's JSON.
func (in *Input) chatCredentialField() string {
	s := in.PlatformSection
	switch {
	case !in.Installation.AgentPlatform || !s.AIChat || s.ChatCredentials:
		return ""
	case s.ChatProvider == chatProviderVertex:
		return fieldChatGoogleCredentials
	}
	return fieldChatKey
}
